...
def main():
    ...
    led.value(1)
    i2c = I2C(0, scl=Pin(5), sda=Pin(4))    # GP5をSSD1306のSCL,GP4をSDAに接続
    i2c.writeto_mem(ssd1306, d_mode_i, d_init) 
    i2c.writeto_mem(ssd1306, d_mode_i, d_home)
    for x in range(8)[::-1]:
        for y in range(16):
            i = ord(disp_port[y][x]) - 32
            if i > 0 and i*8+8 <= len(d_font):
                i2c.writeto_mem(ssd1306, d_mode_w, d_font[i*8:i*8+8])
            else:
                i2c.writeto_mem(ssd1306, d_mode_w, d_font[0:8]) 
    sleep(3)
    for y in range(8)[::-1]:
        for x in range(16)[::-1]:
            ...
            if i > 0 and i*8+8 <= len(d_font):
                ...
                i2c.writeto_mem(ssd1306, d_mode_w, font)
            else:
                i2c.writeto_mem(ssd1306, d_mode_w, d_font[0:8])
    sleep(3)
